...
function getDatabase(): SupabaseClient {
  if (!client) {
    ...
    client = createClient(config.supabaseUrl, config.supabaseServiceRoleKey, { auth: { autoRefreshToken: false, persistSession: false } });
  }
  return client;
}
function throwIfError(error: { message: string } | null): void { if (error) throw new Error("Supabase request failed: " + error.message); }
export async function createSanction(input: NewSanction): Promise<Sanction> {
  const { data, error } = await getDatabase().from("sanctions").insert(input).select("*").single<Sanction>();
  throwIfError(error); if (!data) throw new Error("Supabase did not return the created sanction."); return data;
}
export async function updateSanction(id: string, patch: Partial<Pick<Sanction, "status" | "dm_sent" | "dm_error">>): Promise<void> {
  const { error } = await getDatabase().from("sanctions").update(patch).eq("id", id); throwIfError(error);
}
export async function removeWarning(id: string, moderatorId: string): Promise<Sanction> {
  const { data, error } = await getDatabase().from("sanctions").update({ status: "removed", removed_at: new Date().toISOString(), removed_by: moderatorId }).eq("id", id).eq("type", "warning").eq("status", "applied").select("*").single<Sanction>();
  throwIfError(error); if (!data) throw new Error("Warning not found or already removed."); return data;
}
export async function removeActiveTimeouts(guildId: string, memberId: string, moderatorId = "system"): Promise<Sanction[]> {
  const { data, error } = await getDatabase().from("sanctions").update({ status: "removed", removed_at: new Date().toISOString(), removed_by: moderatorId }).eq("guild_id", guildId).eq("member_id", memberId).eq("type", "timeout").eq("status", "applied").gt("expires_at", new Date().toISOString()).select("*").returns<Sanction[]>();
